...
void setMotorSpeed(int16_t speed) {
	if (speed > 0) {
		// cero tambien a efectos practicos  auque es neutral
		//es positivo
// canal uno en ciclos
		//buscar forma de ponerle valores absolutos
...
// es negativo
		...
		// canal 3 en ciclos
		__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, speed * (-1));
	}
}
...
void setServoPos(float ang){
    ...
    uint16_t grad  = 11.11*ang+500;
 	 __HAL_TIM_SET_COMPARE(&htim3,TIM_CHANNEL_4,grad);
	}
}
...
ini_acelerometro();
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		//1 leer eje y del acelerometro
		int16_t ejeY = readAccel(1);
		//2 escalar valor leido
		// 3 LLamar a set motor speed con una velocidad proporcional a la aceleración
		setMotorSpeed(4*ejeY);
		//5 leer el x del acelerometro
		int16_t ejeX = readAccel(0);
		// 6 Escalar el valor a grados del servo
		float grad  = ((ejeX+1000)* (180.0f/2000.0f));
		// llamar a setServoPOs
setServoPos(grad);
